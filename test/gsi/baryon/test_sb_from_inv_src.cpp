...
RC_GTEST_FIXTURE_PROP(BaryonAPUCTest, sb_from_inv_src, ()) {
  ...
  baryon_foreach_unmasked_section(mask, section, {
    for (size_t i = 0; i < num_vrs; i += 1) {
      memcpy(&(*expected[i])[section], &(*vrs[i])[section], BARYON_WORDLINE_SIZE);
    }
  });

  baryon_sb_from_inv_src(apuc, mask, vrs, num_vrs, src, src_type);
  for (size_t i = 0; i < num_vrs; i += 1) {
