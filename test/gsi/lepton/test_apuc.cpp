...
TEST_F(LeptonAPUCTest, foreach_bank_plat) {
  for (size_t bank = 0; bank < 4; bank += 1) {
    size_t num_plats = 0;
    ...
    ASSERT_EQ(num_plats, LEPTON_NUM_LGL_PLATS)
        << "Expected num_plats to be " << LEPTON_NUM_LGL_PLATS
        << " but was " << num_plats;
  }
}

TEST_F(LeptonAPUCTest, foreach_masked_section) {
