...
RC_GTEST_FIXTURE_PROP(LeptonAPUCTest, sb_cond_eq_inv_src, ()) {
  ...
  for (size_t i = 0; i < num_vrs; i += 1) {
    ...
    memcpy(expected[i], vrs[i], LEPTON_VR_SIZE);
  }
  ...
  lepton_sb_cond_eq_inv_src(apuc, mask, vrs, num_vrs, src, src_type);
  for (size_t i = 0; i < num_vrs; i += 1) {
