...
uint16_t *alloc_i16s(unsigned int count) {
  uint16_t *xs = malloc(count * sizeof(uint16_t));
  return xs;
}
...
gsi_prod_status_t lepton_test_matvec_device(void *in_buf, void *out_buf) {
  ...
  my_dma_l1_to_l4(C_l4, 1, L1_reg_C);
  gvml_exit();
  return 0;
}
...
void lepton_test_matvec_host() {
  ...
  uint16_t *a = malloc(a_size);
  initialize_A(k, a);

  uint16_t *b = malloc(b_size);
  initialize_B(k, n, b);

  uint16_t *c = malloc(c_size);
  ...
  if (gdl_context_alloc(ctx_id, 0, NULL, NULL)) {
    goto clean_up;
  }
  ...
  lepton_test_matvec_device((void *) cmn_struct_mem_handle,
  ...
  for (size_t i = 0; i < n; i += 1) {
    ...
    if (actual_value != expected_value) {
      LEPTON_FAIL_TEST(
          "(HOST) Error in C computation in element %zu, Ref[i] = %u, "
...
void lepton_test_matvec() {
  lepton_test_matvec_host();
}
